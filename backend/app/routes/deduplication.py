...
@router.post("/check")
def check_duplicate(request: DeduplicationRequest):
    ...
    result = process_event(request.text, request.event_data)
    return result
...
def check_similarity(request: DeduplicationRequest):
    ...
    # This endpoint takes two texts and returns their similarity score
    # Expects: {"text": "first headline", "event_data": {"second_text": "second headline"}}
    text1 = request.text
    ...
    return {
        "text1": text1,"text2": text2,"similarity_score": float(similarity),"threshold": 0.85,
